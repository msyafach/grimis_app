...
class IdentifikasiRisikoCreate(IdentifikasiRisikoBase):
    ...
    generation_id: Optional[str] = Field(None, description="ID of the risk statement generation session")
    statement_id: Optional[str] = Field(None, description="ID of the selected risk statement")
...
class IdentifikasiRisikoResponse(IdentifikasiRisikoBase):
    ...
    created_at: datetime = Field(default_factory=datetime.utcnow)
    created_by: str = Field(..., description="ID of the user who created the risk identification")
    created_by_name: str = Field(..., description="Full name of the user who created the risk identification")
    updated_at: Optional[datetime] = None
...
class AnalisisRisikoResponse(AnalisisRisikoBase):
    id: str
    created_at: datetime = Field(default_factory=datetime.utcnow)
    created_by: str = Field(..., description="ID of the user who created the risk analysis")
    created_by_name: str = Field(..., description="Full name of the user who created the risk analysis")
    updated_at: Optional[datetime] = None
    updated_by: Optional[str] = None
    last_updated: Optional[str] = Field(None, description="Tracks which risk type was last updated (INHERIT/RESIDUAL/TREATED/ACTUAL)")
...
class EvaluasiRisikoBase(BaseModel):
    ...
    jenis_pengendalian: Optional[str] = Field(None, description="Type of control (Mengurangi kemungkinan/Mengurangi dampak)")

class EvaluasiRisikoCreate(EvaluasiRisikoBase):
    # Optional fields for AI-generated root causes
    generation_id: Optional[str] = Field(None, description="ID of the root cause generation session")
    root_cause_id: Optional[str] = Field(None, description="ID of the selected root cause")
...
class EvaluasiRisikoResponse(BaseModel):
    ...
    jenis_pengendalian: Optional[str] = Field(None, description="Type of control (Mengurangi kemungkinan/Mengurangi dampak)")
    komentar: Optional[List[KomentarResponse]] = Field([], description="Comments on this evaluation")
    rtp_count: str = Field("0/0", description="Format: 'realized/total' - Number of RTPs that have been realized out of total RTPs")
...
class RTPResponse(RTPBase):
    ...
    created_by_name: str = Field(..., description="Full name of the user who created the RTP")
    updated_by: Optional[str] = None
    verified_at: Optional[datetime] = None
    verified_by: Optional[str] = None
    komentar: Optional[List[KomentarResponse]] = []
    attachments: Optional[List[AttachmentResponse]] = Field(default_factory=list, description="List of file attachments for this RTP")
...
class PetaRisikoKategoriResponse(PetaRisikoKategoriBase):
    id: str
    template_id: str = Field(..., description="ID of the template this category belongs to")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: Optional[datetime] = None
...
class PetaRisikoKlasifikasiResponse(PetaRisikoKlasifikasiBase):
    id: str
    template_id: str = Field(..., description="ID of the template this classification belongs to")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: Optional[datetime] = None
...
class PetaRisikoMatriksResponse(PetaRisikoMatriksBase):
    id: str
    template_id: str = Field(..., description="ID of the template this matrix belongs to")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: Optional[datetime] = None
...
class PetaRisikoMatriksHeatmapBase(BaseModel):
    ...
    kode_warna: Optional[str] = Field(None, description="Color code for the heatmap cell")
...
class PetaRisikoMatriksHeatmapResponse(PetaRisikoMatriksHeatmapBase):
    id: str
    template_id: str = Field(..., description="ID of the template this heatmap belongs to")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: Optional[datetime] = None
...
class RiskStatementGenerationRequest(BaseModel):
    ...
    count: Optional[int] = Field(5, description="Number of risk statements to generate (default: 5)")
...
class RiskStatement(BaseModel):
    ...
    tag: Optional[str] = Field("NORMAL", description="Tag for the risk statement (NORMAL/FRAUD)")
...
class RiskStatementGenerationResponse(BaseModel):
    ...
    statements: List[RiskStatement] = Field(..., description="List of generated risk statements")
...
class RootCauseGenerationRequest(BaseModel):
    ...
    identifikasi_risiko_id: str = Field(..., description="ID of the risk identification")
...
class RootCause(BaseModel):
    ...
    jenis_pengendalian: str = Field(..., description="Type of control (Mengurangi kemungkinan/Mengurangi dampak)")
    
class RootCauseGenerationResponse(BaseModel):
    """Schema for root cause generation response"""
    generation_id: str = Field(..., description="ID of the generation session")
    root_causes: List[RootCause] = Field(..., description="List of generated root causes")
...
class RiskScoreResponse(BaseModel):
    ...
    level_risiko: Optional[int] = Field(None, description="Risk level calculated from score")
    memenuhi: Optional[bool] = Field(None, description="Whether risk meets risk appetite")
    use_risk: Optional[str] = Field(None, description="Current risk type in use (I/R/T/A)")
    kriteria_kemungkinan: Optional[Dict[str, Any]] = Field(None, description="Likelihood criteria details")
    kriteria_dampak: Optional[Dict[str, Any]] = Field(None, description="Impact criteria details")
    updated_by: Optional[str] = Field(None, description="User ID who last updated the risk scores")
    last_updated: Optional[str] = Field(None, description="Risk type that was last updated (INHERIT/RESIDUAL/TREATED/ACTUAL)")

class RiskScoreUpdate(BaseModel):
    """Schema for updating risk scores"""
    skor_kemungkinan: Union[float, int] = Field(..., description="Likelihood score to update")
...
class ApprovalData(BaseModel):
    ...
    is_approved: bool = Field(..., description="Whether the risk identification is approved")
...
class ApprovalAction(BaseModel):
    ...
    kemungkinan_id: Optional[str] = Field(None, description="ID of likelihood criteria (required if approving)")
