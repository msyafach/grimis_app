...
async def create_struktur_organisasi(
    struktur: StrukturOrganisasiCreate, current_user: dict = Depends(get_current_user)
):
    """
    ...
    existing = await db.struktur_organisasi.find_one(
        {"kode": struktur.kode, "id_instansi": struktur.id_instansi}
    )
    if existing:
        raise HTTPException(
            status_code=400, detail="Code already exists for this institution"
        )

    # If kode_induk is provided, validate parent exists
    if struktur.kode_induk:
        parent = await db.struktur_organisasi.find_one(
            {"kode": struktur.kode_induk, "id_instansi": struktur.id_instansi}
        )
        if not parent:
            raise HTTPException(
    ...
    # Ensure jenis_konteks is a list even if not provided
    if "jenis_konteks" not in struktur_dict or struktur_dict["jenis_konteks"] is None:
        struktur_dict["jenis_konteks"] = []
...
async def get_available_induk_units(
    id_instansi: str, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        if current_user["role"] == UserRole.SUPER_ADMIN:
            ...
            async for unit in db.induk_unit_kerja.find({"id_instansi": id_instansi}):
                induk_units.append(
                    {"id": str(unit["_id"]), "nama_induk_unit": unit["nama_induk_unit"]}
                )
            return induk_units
        ...
        if user_units:
            ...
            try:
                query = {"_id": {"$in": object_ids}, "id_instansi": id_instansi}
...
async def get_struktur_organisasi(
    ...
    id_induk_unit_kerja: Optional[str] = Query(
        None, description="Parent Work Unit ID (optional)"
    ),
    ...
    async for struktur in db.struktur_organisasi.find(query).sort("kode", 1):
        ...
        instansi = await db.instansi.find_one(
            {"_id": ObjectId(struktur["id_instansi"])}
        )
        if instansi:
        ...
        if "parent_induk_id" in struktur:
            parent_unit = await db.induk_unit_kerja.find_one(
                {"_id": ObjectId(struktur["parent_induk_id"])}
            )
...
async def get_struktur_organisasi_by_id(
    struktur_id: str, current_user: dict = Depends(get_current_user)
):
    """
...
async def update_struktur_organisasi(
    ...
    if struktur.id_induk_unit_kerja is not None and current_user["role"] not in [
        UserRole.SUPER_ADMIN,
        UserRole.ADMIN_KLP,
    ]:
        raise HTTPException(
            status_code=403,
            detail="Only SUPER_ADMIN and ADMIN_KLP can update parent work unit",
        )
    ...
    if "nama" in update_data or "kode" in update_data:
        ...
        if induk_unit:
            ...
            if induk_update:
                ...
                if "kode" in update_data:
                    # Get all child units
                    async for child in db.induk_unit_kerja.find(
                        {"parent_id": existing["id_induk_unit_kerja"]}
                    ):
                        await db.induk_unit_kerja.update_one(
                            {"_id": child["_id"]},
                            {
    ...
    if (
        "id_induk_unit_kerja" in update_data
        and update_data["id_induk_unit_kerja"] != existing["id_induk_unit_kerja"]
    ):
    ...
    await db.struktur_organisasi.update_one(
        {"_id": ObjectId(struktur_id)}, {"$set": update_data}
    )
    ...
    if "parent_induk_id" in updated:
        parent_unit = await db.induk_unit_kerja.find_one(
            {"_id": ObjectId(updated["parent_induk_id"])}
        )
        if parent_unit and "nama_induk_unit" in parent_unit:
        ...
        # Fallback to current induk_unit_kerja
        induk_unit = await db.induk_unit_kerja.find_one(
            {"_id": ObjectId(updated["id_induk_unit_kerja"])}
        )
        if induk_unit:
...
async def delete_struktur_organisasi(
    ...
    has_children = await db.struktur_organisasi.find_one(
        {"kode_induk": existing["kode"], "id_instansi": existing["id_instansi"]}
    )
    if has_children:
        raise HTTPException(
            status_code=400, detail="Cannot delete structure with child structures"
        )
    ...
    risk_maps = await db.peta_risiko_template.find_one(
        {"id_induk_unit_kerja": induk_unit_id}
    )
    if risk_maps:
        has_dependent_data = True

    # Check for risk identifications using this induk_unit_kerja
    risk_identifications = await db.identifikasi_risiko.find_one(
        {"id_induk_unit_kerja": induk_unit_id}
    )
...
async def assign_users_to_struktur(
    ...
    if current_user["role"] not in [UserRole.SUPER_ADMIN, UserRole.ADMIN_KLP]:
        raise HTTPException(
            status_code=403, detail="Only SUPER_ADMIN and ADMIN_KLP can assign users"
        )
    ...
    for user_id in users.user_ids:
        ...
        assigned_users.append(
            UserInStruktur(user_id=str(user["_id"]), role=user["role"]).dict()
        )

    # Update structure with assigned users
    await db.struktur_organisasi.update_one(
        {"_id": ObjectId(struktur_id)},
        {"$set": {"assigned_users": assigned_users, "updated_at": datetime.utcnow()}},
    )
...
async def get_users_in_struktur(
    struktur_id: str, current_user: dict = Depends(get_current_user)
):
    ALLOWED_ROLES = [
        UserRole.SUPER_ADMIN,
            ...
            users.append(
                {
                    ...
                    "assigned_at": assigned_user.get("assigned_at"),
                }
            )
    return users
...
async def add_jenis_konteks_to_struktur(
    ...
    induk_unit = await db.induk_unit_kerja.find_one(
        {"_id": ObjectId(result["id_induk_unit_kerja"])}
    )
...
@router.get(
    "/{struktur_id}/jenis-konteks/{jenis_konteks_id}",
    response_model=JenisKonteksInStruktur,
)
async def get_jenis_konteks_by_id(
    ...
    if not jenis_konteks:
        raise HTTPException(
            status_code=404, detail="Context type not found in this structure"
        )
...
async def delete_jenis_konteks(
    ...
    konteks_count = await db.konteks.count_documents(
        {"id_jenis_konteks": jenis_konteks_id}
    )
    if konteks_count > 0:
        raise HTTPException(
            status_code=400,
    ...
    if result.modified_count == 0:
        raise HTTPException(
            status_code=404, detail="Context type not found or couldn't be deleted"
        )
...
async def update_jenis_konteks(
    ...
    result = await db.struktur_organisasi.update_one(
        {"_id": ObjectId(struktur_id), "jenis_konteks.id": jenis_konteks_id},
        {"$set": {"jenis_konteks.$": update_data, "updated_at": datetime.utcnow()}},
    )

    if result.modified_count == 0:
        raise HTTPException(
            status_code=404, detail="Context type not found or couldn't be updated"
        )

    # Get the updated structure
    updated_struktur = await db.struktur_organisasi.find_one(
        {"_id": ObjectId(struktur_id)}
    )
    updated_jenis_konteks = next(
        (
            jk
            for jk in updated_struktur.get("jenis_konteks", [])
            if jk["id"] == jenis_konteks_id
        ),
        None,
    )
    ...
    if not updated_jenis_konteks:
        raise HTTPException(
            status_code=404, detail="Context type not found after update"
        )
