...
@router.get("/check-email", response_model=Optional[UserResponse])
async def check_user_email(email: str = Query(...)):
    """
    Check if a user exists by their email.
    Used in the organizational structure user registration wizard.
    """
...
async def register_user(
    ...
    if current_user["role"] not in [UserRole.SUPER_ADMIN, UserRole.ADMIN_KLP]:
        raise HTTPException(
            status_code=403, detail="Not enough permissions to register new users"
        )
    ...
    if current_user["role"] == UserRole.ADMIN_KLP and user.role == UserRole.SUPER_ADMIN:
        raise HTTPException(
            status_code=403, detail="ADMIN_KLP cannot create SUPER_ADMIN accounts"
        )
    ...
    if user.role != UserRole.SUPER_ADMIN:
        ...
        if not instansi:
            raise HTTPException(
                status_code=404, detail="Specified institution not found"
            )
        ...
        for unit_id in user.induk_unit_kerja_ids:
            try:
                ...
                if not induk_unit:
                    raise HTTPException(
                        status_code=404, detail=f"Parent work unit not found: {unit_id}"
                    )
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid parent work unit ID: {unit_id}"
                )
...
async def get_current_user_info(current_user: dict = Depends(get_current_user)):
    ...
    user["induk_unit_kerja_ids"] = user.get("induk_unit_kerja_ids") or user.get(
        "id_induk_unit_kerja"
    )
...
async def get_all_users(
    current_user: dict = Depends(get_current_user), instansi_id: Optional[str] = None
):
...
async def update_user(
    user_id: str, user: UserUpdate, current_user: dict = Depends(get_current_user)
):
    """
    Update user data.
    ...
    if existing.get("is_root", False) and current_user["id"] != user_id:
        raise HTTPException(
            status_code=403, detail="Root users can only be edited by themselves"
        )
    ...
    if current_user["role"] == UserRole.SUPER_ADMIN:
        ...
        if existing.get("instansi_id") != current_user_data.get("instansi_id"):
            raise HTTPException(
                status_code=403, detail="You can only update users in your institution"
            )
        ...
        if user.instansi_id is not None and user.instansi_id != existing.get(
            "instansi_id"
        ):
            raise HTTPException(
                status_code=403, detail="You cannot change a user's institution"
            )
        ...
        if user.induk_unit_kerja_ids is not None:
            ...
            for unit_id in user.induk_unit_kerja_ids:
                try:
                    ...
                    if induk_unit.get("id_instansi") != current_user_data.get(
                        "instansi_id"
                    ):
                        raise HTTPException(
                            status_code=400,
    ...
    if "is_root" in update_data and not current_user.get("is_root", False):
        raise HTTPException(
            status_code=403, detail="Only root users can modify the is_root flag"
        )
    ...
    if "induk_unit_kerja_ids" in update_data and update_data["induk_unit_kerja_ids"]:
        ...
        if (
            not current_last_unit
            or current_last_unit not in update_data["induk_unit_kerja_ids"]
        ):
            # Set first unit in the new list as the default
            update_data["last_induk_unit_kerja_id"] = update_data[
    ...
    if "role_ids" in update_data:
        ...
        for role_id in new_role_ids - old_role_ids:
            try:
                ...
                if role:
                    await db.roles.update_one(
                        {"_id": ObjectId(role_id)}, {"$addToSet": {"user_ids": user_id}}
                    )
            except:
                continue
        ...
        for role_id in old_role_ids - new_role_ids:
            try:
                await db.roles.update_one(
                    {"_id": ObjectId(role_id)}, {"$pull": {"user_ids": user_id}}
                )
            except:
                continue
    ...
    if updated.get("instansi_id"):
        try:
            instansi = await db.instansi.find_one(
                {"_id": ObjectId(updated["instansi_id"])}
            )
            if instansi:
...
async def delete_user(user_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # SUPER_ADMIN rule
        ...
        if user_to_delete["role"] == UserRole.SUPER_ADMIN:
            raise HTTPException(
                status_code=403, detail="Cannot delete other SUPER_ADMIN accounts"
            )
        ...
        struktur_cursor = db.struktur_organisasi.find(
            {
                ...
                "id_induk_unit_kerja": {
                    "$in": user_to_delete.get("induk_unit_kerja_ids", [])
                },
                "assigned_users": {"$elemMatch": {"user_id": str(current_user["id"])}},
            }
        )
...
async def update_user_preferences(
    ...
    if last_induk_unit_kerja_id:
        ...
        induk_unit = await db.induk_unit_kerja.find_one(
            {"_id": ObjectId(last_induk_unit_kerja_id)}
        )
        if not induk_unit:
...
async def check_and_update_work_unit(current_user: dict = Depends(get_current_user)):
    """
    ...
    is_locked_to_instansi = user.get("role") != UserRole.SUPER_ADMIN and user.get(
        "instansi_id"
    )
    ...
    if current_induk_unit_kerja_id:
        try:
            current_unit = await db.induk_unit_kerja.find_one(
                {"_id": ObjectId(current_induk_unit_kerja_id)}
            )
            # If unit doesn't exist or belongs to wrong institution, we need to update
            if (
                not current_unit
                or current_unit.get("id_instansi") != effective_instansi_id
            ):
                update_needed = True
    ...
    if update_needed:
        ...
        try:
            ...
            if (
                is_locked_to_instansi
                and user.get("induk_unit_kerja_ids")
                and len(user.get("induk_unit_kerja_ids", [])) > 0
            ):
            ...
            first_unit = await db.induk_unit_kerja.find_one(
                {"id_instansi": effective_instansi_id, "parent_id": None}
            )
...
async def get_current_user_debug(current_user: dict = Depends(get_current_user)):
    ...
    for unit_id in user.get("induk_unit_kerja_ids", []):
        try:
            ...
            induk_units_info.append(
                {"id": unit_id, "error": "Invalid unit ID or not found"}
            )
    ...
    # Include API access
    api_access = {"induk_unit_kerja_access": [], "struktur_organisasi_access": []}

    # Check access to both APIs if an instansi is assigned
    if user.get("instansi_id"):
        try:
            # Check access to induk-unit-kerja endpoint
            res = await db.induk_unit_kerja.find(
                {"id_instansi": user["instansi_id"]}
            ).to_list(10)
        ...
        try:
            ...
            structs = await db.struktur_organisasi.find(
                {"id_instansi": user["instansi_id"]}
            ).to_list(10)
            api_access["struktur_organisasi_access"] = [
                str(struct["_id"]) for struct in structs
            ]
        except Exception as e:
...
async def update_self(
    user_data: UserSelfUpdate, current_user: dict = Depends(get_current_user)
):
    """
