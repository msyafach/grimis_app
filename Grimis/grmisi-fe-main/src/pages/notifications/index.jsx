...
const NotificationsIndex = () => {
  ...
  const handleMakeAsRead = async (id = null) => {
    try {
      ...
      if (id) {
        await axios.put(
          API_ENDPOINTS.markNotificationAsRead(id),
          {},
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        ...
        await axios.put(
          API_ENDPOINTS.markAllNotificationsAsRead,
          {},
          {
            headers: { Authorization: `Bearer ${token}` },
          },
                    ...
                    <tbody>
                      {notifications.map((notification) => (
                        <tr
                          key={notification.id}
                          className={
                            !notification.is_read ? "bg-soft-primary" : ""
                          }
                        >
                          <td className="w-50">
                            ...
                            <div className="d-flex justify-content-end gap-2">
                              {!notification.is_read && (
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleMakeAsRead(notification.id)
                                  }
                                  title="Mark as Read"
                                >
