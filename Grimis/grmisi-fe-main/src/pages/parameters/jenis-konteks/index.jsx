...
import { FiPlus, FiEye, FiEdit3, FiTrash2, FiMoreHorizontal } from 'react-icons/fi';
...
const JenisKonteks = () => {
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            const msg = err?.response?.data?.detail || 'Gagal menambahkan jenis konteks';
            showToast('error', msg);
        }
    };
    ...
    const handleDelete = async () => {
        try {
            await axios.delete(
                API_ENDPOINTS.deleteJenisKonteks(selectedItem.id),
                { headers: { Authorization: `Bearer ${token}` } }
            );
            ...
            const msg = err?.response?.data?.detail || 'Gagal menghapus jenis konteks';
            showToast('error', msg);
        }
    };
    ...
    return (
        <>
            ...
            <div className="main-content" style={{ minHeight: 'calc(100vh - 60px)' }}>
                <div className="card">
                    <div className="card-header d-flex justify-content-between align-items-center">
                        ...
                        >
                            <FiPlus className="me-1" /> Tambah Jenis Konteks
                        </Button>
                    </div>
                                    ...
                                    <tbody>
                                        {filteredData.map((item) => (
                                            <tr key={item.id}>
                                                    ...
                                                    <div className="position-relative">
                                                        ...
                                                        {activeDropdown === item.id && (
                                                            <div
                                                                ...
                                                                style={{ zIndex: 1050, minWidth: '150px' }}
                                                            >
                                                                <button className="dropdown-item" onClick={() => openDetailModal(item)}>
                                                                ...
                                                                {user?.role === 'SUPER_ADMIN' && (
                                                                    <button className="dropdown-item text-danger" onClick={() => openDeleteModal(item)}>
                                                ...
                                                <td>{item.kode}</td>
                                                <td>
                                                    <span className={`badge ${item.jenis === 'SASARAN' ? 'bg-primary' : 'bg-info'}`}>
                                                        {item.jenis}
                                                    </span>
                                                </td>
                                                <td>{item.nama}</td>
                ...
                <Modal.Body>
                    <Form>
                        <Form.Group className="mb-3">
                            <Form.Label className="fw-bold">SASARAN / PROBIS</Form.Label>
                            <div>
                                <Form.Check
                                    inline
                                    type="radio"
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label className="fw-bold">KODE *</Form.Label>
                            <Form.Control
                                type="text"
                                value={form.kode}
                                onChange={(e) => setForm({ ...form, kode: e.target.value })}
                                placeholder="Contoh: SS, SP, SK, PB"
                            />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control
                                ...
                                placeholder="Contoh: Sasaran Strategis"
                            />
                        </Form.Group>
                    </Form>
                </Modal.Body>
            ...
            <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
                ...
                <Modal.Body>
                    <Form>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label className="fw-bold">KODE *</Form.Label>
                            <Form.Control type="text" value={form.kode} disabled />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            <Form.Label className="fw-bold">NAMA *</Form.Label>
                            <Form.Control
            ...
            <Modal show={showDetailModal} onHide={() => setShowDetailModal(false)}>
                ...
                <Modal.Body>
                    {selectedItem && (
                        <div>
                            <div className="mb-3">
                                <strong>SASARAN / PROBIS</strong>
                                <div className="mt-1">
                                    <span className={`badge ${selectedItem.jenis === 'SASARAN' ? 'bg-primary' : 'bg-info'}`}>
                                        {selectedItem.jenis}
                                    </span>
                                </div>
                            </div>
                            <div className="mb-3">
                                <strong>KODE</strong>
                                <p className="mb-0 mt-1">{selectedItem.kode}</p>
                            </div>
                            <div className="mb-3">
                                <strong>NAMA</strong>
                                <p className="mb-0 mt-1">{selectedItem.nama}</p>
                            </div>
                        </div>
                    )}
                </Modal.Body>
            ...
            <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)}>
                <Modal.Header closeButton>
                    <Modal.Title>Data Jenis Konteks</Modal.Title>
                </Modal.Header>
                <Modal.Body>
