...
const JenisPenyebab = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        API_ENDPOINTS.getJenisPenyebabAll(idInstansi),
        { headers: { Authorization: `Bearer ${token}` } },
      );
      setData(response.data || []);
    } catch (err) {
      console.error("Error fetching jenis penyebab:", err);
      showToast("error", "Gagal memuat data jenis penyebab");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      await axios.post(
        API_ENDPOINTS.postJenisPenyebab,
        {
          kode: form.kode,
          nama: form.nama,
      ...
      const msg =
        err?.response?.data?.detail || "Gagal menambahkan jenis penyebab";
      showToast("error", msg);
    }
  };
  ...
  const handleEdit = async () => {
    ...
    try {
      await axios.put(
        API_ENDPOINTS.updateJenisPenyebab(selectedItem.id),
        { nama: form.nama },
        { headers: { Authorization: `Bearer ${token}` } },
      );
      showToast("success", "Jenis penyebab berhasil diubah");
      setShowEditModal(false);
      fetchData();
    } catch (err) {
      const msg = err?.response?.data?.detail || "Gagal mengubah jenis penyebab";
      showToast("error", msg);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(API_ENDPOINTS.deleteJenisPenyebab(selectedItem.id), {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const msg =
        err?.response?.data?.detail || "Gagal menghapus jenis penyebab";
      showToast("error", msg);
    }
  };
  ...
  return (
    <>
      <PageHeader title="Jenis Penyebab" />
      <div className="main-content" style={{ minHeight: "calc(100vh - 60px)" }}>
        <div className="card">
          <div className="card-header d-flex justify-content-between align-items-center">
            <h5 className="card-title mb-0">Data Jenis Penyebab</h5>
            <Button
              variant="primary"
              size="sm"
              onClick={() => {
      ...
      <Modal show={showAddModal} onHide={() => setShowAddModal(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Tambah Jenis Penyebab</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Ubah Jenis Penyebab</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group className="mb-4">
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Detil Jenis Penyebab</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedItem && (
            <div>
              <div className="mb-4">
                ...
                <Form.Control
                  type="text"
                  className="form-control-sm rounded-2 bg-light text-uppercase"
