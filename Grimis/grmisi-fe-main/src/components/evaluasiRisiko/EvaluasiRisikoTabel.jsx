            ...
            try {
                ...
                const combinedData = await Promise.all(
                    identifikasiData.map(async (item) => {
                        ...
                        try {
                            const konteksSasaranResponse = await axios.get(API_ENDPOINTS.getKonteksSasaranById(item.id_konteks_sasaran), {
                                headers: { Authorization: `Bearer ${token}` }
                            });
                            namaSasaran = konteksSasaranResponse.data?.nama || "";

                            const konteksProbisResponse = await axios.get(API_ENDPOINTS.getKonteksProbisById(item.id_konteks_probis), {
                                headers: { Authorization: `Bearer ${token}` }
                            });
                        ...
                        try {
                            const analisisResponse = await axios.get(API_ENDPOINTS.getAnalisisRisikoByIdentifikasiRisikoId(item.id), {
                                headers: { Authorization: `Bearer ${token}` }
                            });
