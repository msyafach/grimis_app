...
const SelectDropdownCustom = ({ options, selectedOption, onSelectOption, className, defaultSelect }) => {
    ...
    return (
        <div className={`select-dropdown select-dropdown-sm ${className ? className : ""} ${openUpwards ? 'open-upwards' : ''} rounded-3`} ref={dropdownRef}>
            <div className={"select-box select-box-sm rounded-4"} onClick={toggleDropdown}>
                {localSelectedOption ? (
                    <div className="d-flex align-items-center">
                        ...
                        <span className={`badge ${localSelectedOption.color} px-2 py-1`}>
                    ...
                    <ul>
                        {filteredOptions.length > 0 ? (
                            filteredOptions.map((option) => (
                                ...
                                >
                                    <span className="me-2">{option.label}</span> {/* Deskripsi */}
                                    <span className={`badge ${option.color} px-2 py-1`}>{option.persentase}</span> {/* Persentase dalam Badge */}
                                </li>
                            ))
                        ) : (
...
// SelectDropdownCustom.defaultProps = {
//     currentSelect: () => { }
// };