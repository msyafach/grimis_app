    ...
    if (menu.name === "parameters" && menu.dropdownMenu) {
      return {
        ...
        dropdownMenu: menu.dropdownMenu.filter((item) => {
          // Admin roles: SUPER_ADMIN, ADMIN_KLP
          if (isAdmin) {
            // Admin menu as per Image 1
            const adminItems = [
              "Setting Matriks Risiko",
              "Pengisian Kategori dan Kriteria Risiko",
              "Pengisian Kategori dan Kriteria Dampak",
              "Struktur Organisasi",
              "Pemilihan Warna Matriks Risiko",
              "Jenis Penyebab",
              "Kategori Risiko",
              "Jenis Konteks",
              "Kamus Risiko",
            ...
            // Non-admin (Pemilik/Pengelola) menu as per Image 2
            const userItems = [
              "Kamus Risiko",
              "Usulan Kamus",
              "Usulan Konteks",
  ...
  return (
    <>
      {filteredMenuList.map(({ dropdownMenu, id, name, path, icon }) => {
        ...
        const isAnyChildActive = dropdownMenu.some(
          (sub) =>
            pathName.startsWith(sub.path) ||
            (sub.subdropdownMenu &&
              sub.subdropdownMenu.some((subSub) =>
          ...
          >
            ...
            >
              {dropdownMenu.map(({ id, name, path, subdropdownMenu }) => {
                ...
                const isAnySubChildActive =
                  subdropdownMenu &&
                  subdropdownMenu.some((subSub) =>
                    pathName.startsWith(subSub.path),
                  );
                const isSubMenuActive =
                  pathName.startsWith(path) || isAnySubChildActive;
                const isSubMenuTriggered =
                  openSubDropdown === x || isSubMenuActive;
