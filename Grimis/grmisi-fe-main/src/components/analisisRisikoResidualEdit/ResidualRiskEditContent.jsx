...
const ResidualRiskEditContent = ({ title = "Skor / Nilai Residual Risk" }) => {
    ...
    const fetchAllData = useCallback(async () => {
        ...
        try {
            ...
            const [indentifikasiRes, kemungkinanRes, dampakRes, residualRes, skorFreqRes, skorDampakRes, matriksRes, attachmentsRes] = await Promise.all([
                ...
                axios.get(API_ENDPOINTS.getAnalisisRisikoResidual(analisisId), { headers }),
            ...
            // Safely set the selected kemungkinan with proper error handling
            try {
                const kemungkinanItem = kemungkinanRes.data.find(k => k.id === residualRes.data.kemungkinan_id);
                setSelectedKemungkinan({
                    label: kemungkinanItem?.deskripsi || 'Tidak ditemukan',
                    value: kemungkinanItem?.id || ''
                });
            } catch (error) {
                console.error("Error setting kemungkinan:", error);
                setSelectedKemungkinan({
            ...
            // Safely set the selected dampak with proper error handling
            try {
                const dampakItem = dampakRes.data.find(d => d.id === residualRes.data.dampak_id);
                setSelectedDampak({
                    label: dampakItem?.deskripsi || 'Tidak ditemukan',
                    value: dampakItem?.id || ''
                });
            } catch (error) {
                console.error("Error setting dampak:", error);
                setSelectedDampak({
                    label: 'Tidak ditemukan',
                    value: ''
                });
            ...
            const kemungkinanKriteria = kemungkinanRes.data.find(
                k => k.id === residualRes.data.kemungkinan_id
            );
            const dampakKriteria = dampakRes.data.find(
                d => d.id === residualRes.data.dampak_id
            );
            ...
            const persentaseKemungkinan = matriksKemungkinan.find(item =>
                parseInt(item.kategori) === parseInt(residualRes.data.skor_kemungkinan)
            )?.value;
            ...
            setSelectedSkorKemungkinan({
                value: residualRes.data.skor_kemungkinan,
                label: skorFreqRes.data.find(item => parseInt(item.key) === parseInt(residualRes.data.skor_kemungkinan))?.value || '',
                color: skorColorMap[residualRes.data.skor_kemungkinan] || 'bg-secondary',
                persentase: persentaseKemungkinan || ''
            });

            const persentaseDampak = matriksDampak.find(item =>
                parseInt(item.kategori) === parseInt(residualRes.data.skor_dampak)
            )?.value;

            setSelectedSkorDampak({
                value: residualRes.data.skor_dampak,
                label: skorDampakRes.data.find(item => parseInt(item.key) === parseInt(residualRes.data.skor_dampak))?.value || '',
                color: skorColorMap[residualRes.data.skor_dampak] || 'bg-secondary',
                persentase: persentaseDampak || ''
            });

            setResidual(residualRes.data);
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Check if there are attachments (existing controls)
            if (attachments.length === 0) {
                setShowWarningModal(true);
            } else {
                // If there are attachments, submit directly
                await submitForm(data);
            }
        } catch (error) {
            console.error("Error preparing submission:", error);
            setErrorMessage(error.response?.data?.detail || "Terjadi kesalahan. Silakan coba lagi.");
            showToast("error", error.response?.data?.detail || 'Terjadi kesalahan. Silakan coba lagi.');
        }
    };
    ...
    const submitForm = async (data) => {
        ...
        try {
            ...
            await axios.put(API_ENDPOINTS.putAnalisisRisikoResidual(analisisId), data, { headers });
            await fetchAllData();
            showToast("success", "Residual Risk Berhasil Diubah!");
        } catch (error) {
            ...
            if (error.response) {
                ...
                showToast("error", "Tidak ada respons dari server. Periksa koneksi internet Anda.");
            } else {
    ...
    const level = residual?.level_risiko || 0;
    ...
    return (
        <div className="card stretch stretch-full">
            ...
            <Modal show={showWarningModal} onHide={() => setShowWarningModal(false)} centered>
                ...
                <Modal.Body>
                    <p>Mohon maaf, nilai residual akan tetap sama dengan nilai sebelumnya karena belum memiliki existing control.</p>
                    <p>Apakah Anda tetap ingin melanjutkan?</p>
                </Modal.Body>
                <Modal.Footer>
...
ResidualRiskEditContent.propTypes = {
    title: PropTypes.string,
};

export default ResidualRiskEditContent;
