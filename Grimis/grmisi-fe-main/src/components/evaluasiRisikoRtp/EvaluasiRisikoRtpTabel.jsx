...
const EvaluasiRisikoRtpTabel = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const rtpRes = await axios.get(API_ENDPOINTS.getRtpDampakByIdentifikasi(identifikasiId), { headers })
    ...
    const handleActionClick = useCallback((action, row) => {
        const basePath = `/pengelolaan-risiko/identifikasi-risiko/edit/${identifikasiId}/rtp`;
        const fromPath = `${basePath}`; 

        switch (action) {
            case 'realisasi':
                navigate(`/pengelolaan-risiko/identifikasi-risiko/edit/${identifikasiId}/rtp/realisasi/${row.id}`, {
                    state: { from: fromPath }
                });
                break;
            case 'view':
                navigate(`/pengelolaan-risiko/identifikasi-risiko/edit/${identifikasiId}/rtp/detail/${row.id}`, {
                    state: { from: fromPath }
                });
                break;
            case 'edit':
                navigate(`/pengelolaan-risiko/identifikasi-risiko/edit/${identifikasiId}/rtp/edit/${row.id}`, {
                    state: { from: fromPath }
                });
                break;
            case 'delete':
    ...
    return (
        <>
            <TableEvaluasiRtp title={"RTP Mengurangi Dampak"} data={evaluasiRtp} columns={columns} />
...
export default EvaluasiRisikoRtpTabel;
