...
const InherentRiskEditContent = ({ title = "Skor / Nilai Inherent Risk" }) => {
    ...
    const [inherent, setInherent] = useState(null);
    ...
    const fetchAllData = useCallback(async () => {
        ...
        try {
            ...
            const [indentifikasiRes, kemungkinanRes, dampakRes, inherentRes, skorFreqRes, skorDampakRes, matriksRes] = await Promise.all([
                ...
                axios.get(API_ENDPOINTS.getAnalisisRisikoInherent(analisisId), { headers }),
            ...
            const selectedKemungkinanOption = {
                label: kemungkinanRes.data.find(k => k.id === inherentRes.data.kemungkinan_id)?.deskripsi || '',
                value: inherentRes.data.kemungkinan_id
            };
            setSelectedKemungkinan(selectedKemungkinanOption);
            
            const selectedDampakOption = {
                label: dampakRes.data.find(d => d.id === inherentRes.data.dampak_id)?.deskripsi || '',
                value: inherentRes.data.dampak_id
            };
            setSelectedDampak(selectedDampakOption);

            // Find kriteria details directly from the arrays we already have
            const kemungkinanKriteria = kemungkinanRes.data.find(
                k => k.id === inherentRes.data.kemungkinan_id
            );
            const dampakKriteria = dampakRes.data.find(
                d => d.id === inherentRes.data.dampak_id
            );
            ...
            const persentaseKemungkinan = matriksKemungkinan.find(item =>
                parseInt(item.kategori) === parseInt(inherentRes.data.skor_kemungkinan)
            )?.value;
            ...
            const currentSkorKemungkinan = {
                value: inherentRes.data.skor_kemungkinan,
                label: skorFreqRes.data.find(item => parseInt(item.key) === parseInt(inherentRes.data.skor_kemungkinan))?.value || '',
                color: skorColorMap[inherentRes.data.skor_kemungkinan] || 'bg-secondary',
                persentase: persentaseKemungkinan || ''
            };
            setSelectedSkorKemungkinan(currentSkorKemungkinan);

            const persentaseDampak = matriksDampak.find(item =>
                parseInt(item.kategori) === parseInt(inherentRes.data.skor_dampak)
            )?.value;

            const currentSkorDampak = {
                value: inherentRes.data.skor_dampak,
                label: skorDampakRes.data.find(item => parseInt(item.key) === parseInt(inherentRes.data.skor_dampak))?.value || '',
                color: skorColorMap[inherentRes.data.skor_dampak] || 'bg-secondary',
                persentase: persentaseDampak || ''
            };
            setSelectedSkorDampak(currentSkorDampak);

            setInherent(inherentRes.data);
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(API_ENDPOINTS.putAnalisisRisikoInherent(analisisId), updateData, { headers });
            
            await fetchAllData();
            showToast("success", "Inherent Risk Berhasil Diubah!")
        } catch (error) {
            console.error("Error updating inherit risk:", error);
    ...
    const level = inherent?.level_risiko || 0;
...
InherentRiskEditContent.propTypes = {
    title: PropTypes.string,
};

export default InherentRiskEditContent;
