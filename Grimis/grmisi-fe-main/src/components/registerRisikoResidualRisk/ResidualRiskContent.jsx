...
const ResidualRiskContent = ({ title = "Skor / Nilai Residual Risk" }) => {
    ...
    // const { idAnalisisRisiko } = useParams();
    ...
    const [status, setStatus] = useState("Memadai");
        ...
        const fetchAllData = async () => {
            ...
            try {
                ...
                if (analisisRisikoResponse.data && analisisRisikoResponse.data.length > 0) {
                    ...
                    setLevel(analisisData.level_risiko_residual || 0);
                    setSelectedKriteriaSkorKemungkinan(analisisData.skor_kemungkinan_residual);
                    setSelectedKriteriaSkorDampak(analisisData.skor_dampak_residual);
                    setStatus(analisisData.memenuhi_residual ? "Memadai" : "Belum Memadai");
                }
    ...
    const handleSave = async (e) => {
        ...
        const payload = {
            ...
            skor_kemungkinan_residual: Number(selectedKriteriaSkorKemungkinan),
            skor_dampak_residual: Number(selectedKriteriaSkorDampak),
        };
        ...
        try {
            ...
            if (selectedIdAnalisisRisiko) {
                ...
                setLevel(res.data.level_risiko_residual || 0);
                setStatus(res.data.memenuhi_residual ? "Memadai" : "Belum Memadai");
            } else {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Berhasil',
                text: 'Data Residual Risk berhasil disimpan',
            });
        } catch (error) {
                    ...
                    {loading ? (
                        ...
                        <div className="d-flex justify-content-center align-items-center">
                            <div className="col-xl-12">
                                ...
                                <div className="row">
                                    <div className="col-lg-12">
                                        <h5 className="fw-bold">Memadai / Belum Memadai</h5>
                                    </div>
                                </div>
                                <div className="row">
                                    <div className="col-lg-1 mb-2">
                                        <span
                                            className={`badge ${status === 'Memadai' ? 'bg-light-success' : 'bg-light-danger'} 
                                            text-${status === 'Memadai' ? 'success' : 'danger'} 
                                            rounded-4 p-2 d-inline-flex justify-content-center align-items-center`}>
                                            <span className="ms-4 me-4 fs-5 text-center">{status}</span>
                                        </span>
                                    </div>
                                </div>
