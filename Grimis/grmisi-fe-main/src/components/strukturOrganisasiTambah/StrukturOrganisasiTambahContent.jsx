...
const StrukturOrganisasiTambahContent = ({
  ...
  useEffect(() => {
    if (selectedIndukUnitKerja && dataIndukUnitKerja.length > 0) {
      const parent = dataIndukUnitKerja.find(
        (item) => item.id_induk_unit_kerja === selectedIndukUnitKerja.value,
      );
      if (parent) {
        setParentSeleraRisiko(parent.selera_risiko);
      } else {
        setParentSeleraRisiko(null);
      }
    } else {
      setParentSeleraRisiko(null);
    }
  }, [selectedIndukUnitKerja, dataIndukUnitKerja]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      formData.selera_risiko > parentSeleraRisiko
    ) {
      setLoading(false);
      showToast(
        "error",
    ...
    try {
      ...
      const submissionData = {
        ...
        jenis_konteks: [], // Sent as empty array as it's no longer needed in this menu
      };

      const headers = { Authorization: `Bearer ${token}` };
      const res = await axios.post(
        API_ENDPOINTS.postStrukturOrganisasi,
        submissionData,
        { headers },
      );
      ...
      if (res.data) {
        ...
        if (newIndukUnitKerjaId) {
          console.log(
            "Creating template for new induk unit kerja:",
            newIndukUnitKerjaId,
          );
          ...
          try {
            ...
            if (templateId) {
              ...
              await axios.post(
                API_ENDPOINTS.syncKriteriaRisikoFromTemplate(
                  idInstansi,
                  newIndukUnitKerjaId,
                  templateId,
                ),
