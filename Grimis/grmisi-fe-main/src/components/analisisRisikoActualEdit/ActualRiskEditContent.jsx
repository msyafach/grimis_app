...
const ActualRiskEditContent = ({ title = "Skor / Nilai Actual Risk" }) => {
    ...
    // Define columns for the monitoring data table
    ...
    const fetchMonitoringData = useCallback(async () => {
        ...
        try {
            ...
            const response = await axios.get(
                API_ENDPOINTS.getAnalisisRisikoMonitoringData(analisisId, tahunId),
                { headers }
            );
    ...
    const fetchAllData = useCallback(async () => {
        ...
        try {
            ...
            const kemungkinanKriteria = kemungkinanRes.data.find(
                k => k.id === actualRes.data.kemungkinan_id
            );
            const dampakKriteria = dampakRes.data.find(
                d => d.id === actualRes.data.dampak_id
            );
            ...
            const persentaseDampak = matriksDampak.find(item =>
                parseInt(item.kategori) === parseInt(actualRes.data.skor_dampak)
            )?.value;
    ...
    }, [fetchMonitoringData, analisisId]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(API_ENDPOINTS.putAnalisisRisikoActual(analisisId), {
            ...
            await fetchAllData();
            showToast("success", "Actual Risk Berhasil Diubah!")
        } catch (error) {
            console.error("Error updating actual risk:", error);
                ...
                <ContentLoaderWrapper loading={loading} error={errorMessage}>
                    {!analisisId && !loading ? (
                        ...
                        <>
                            ...
                            {/* Monitoring Risk Data Section */}
                            <div className="mt-5">
                                <ContentLoaderWrapper loading={loadingMonitoring} error={null}>
                                    {monitoringData.length > 0 ? (
                                        <Table
                                            title={`Lesson Learned Monitoring Risiko`}
                                            data={monitoringData}
                                            columns={monitoringColumns}
                                        />
                                    ) : (
                                        <div className="alert alert-info">
                                            Tidak ada data monitoring risiko untuk tahun {tahunId}
                                        </div>
                                    )}
...
ActualRiskEditContent.propTypes = {
    title: PropTypes.string,
};

export default ActualRiskEditContent;