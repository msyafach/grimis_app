...
const KamusRisikoEditContent = ({ title = "Edit Kamus Risiko" }) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            setFormData({
                ...
                id_kategori_risiko: response.data.id_kategori_risiko,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const token = localStorage.getItem("access_token");
            await axios.put(API_ENDPOINTS.putKamusRisiko(kamusRisikoId), formData, {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchData();
            showToast("success", "Kamus Risiko  Berhasil Diperbarui!");
        } catch (error) {
            const errorResponse = error.response?.data?.detail || "Terjadi kesalahan. Silakan coba lagi.";
            const translatedError = translate(errorResponse);
...
KamusRisikoEditContent.propTypes = {
    title: PropTypes.string
};

export default KamusRisikoEditContent;
