...
const KategoriRisikoTabel = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (idIndukUnitKerja) {
                    ...
                    response = await axios.get(API_ENDPOINTS.getKategoriRisikoByInstansiAndIndukUnitKerja(idInstansi, idIndukUnitKerja), {
                        headers: { Authorization: `Bearer ${token}` },
                    });
                } else {
                    // Otherwise, just filter by instansi
                    response = await axios.get(API_ENDPOINTS.getKategoriRisikoAll(idInstansi), {
                        headers: { Authorization: `Bearer ${token}` },
                    });
                }
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            await axios.delete(API_ENDPOINTS.deleteKategoriRisiko(selectedKategoriRisiko.id), {
                headers: { Authorization: `Bearer ${token}` },
            });

            showToast("success", "Kategori Risiko berhasil dihapus!");
            setKategoriRisiko(prev => prev.filter(item => item.id !== selectedKategoriRisiko.id));
            setShowModal(false);
    ...
    const columns = useMemo(() => getKategoriRisikoColumns(handleActionClick, user), [handleActionClick, user]);
    ...
    return (
        <>
            <Table title="Data Kategori Risiko" data={kategoriRisiko} columns={columns} />
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)} centered>
                ...
                <Modal.Body>
                    {selectedKategoriRisiko && (
                        <>
                            <p>Yakin ingin menghapus kategori risiko ini?</p>
                            <strong>Nama:</strong> {selectedKategoriRisiko.nama}<br />
                            <strong>Kode:</strong> {selectedKategoriRisiko.kode}<br />
                            <strong>Nama KLP:</strong> {selectedKategoriRisiko.nama_klp}<br />
                        </>
                    )}
                </Modal.Body>
                <Modal.Footer>
