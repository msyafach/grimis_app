...
const KriteriaKemungkinanEditContent = ({ title = "Edit Kriteria Risiko Kemungkinan" }) => {
    const { isRemoved, handleRefresh, handleExpand, handleDelete } = useCardTitleActions();
    const navigate = useNavigate();
    const { kriteriaKemungkinanId } = useParams();
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            const response = await axios.get(API_ENDPOINTS.getKriteriaRisikoKemungkinanById(kriteriaKemungkinanId), { headers });
            setFormData({
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const token = localStorage.getItem("access_token");
            await axios.put(API_ENDPOINTS.putKriteriaRisikoKemungkinan(kriteriaKemungkinanId), formData, {
                headers: { Authorization: `Bearer ${token}` },
            });
            showToast("success", "Kriteria Risiko Kemungkinan Berhasil Diperbarui!");
        } catch (error) {
...
KriteriaKemungkinanEditContent.propTypes = {
    title: PropTypes.string,
    resetKey: PropTypes.func.isRequired,
};

export default KriteriaKemungkinanEditContent;